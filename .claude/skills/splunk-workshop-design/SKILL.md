...
The TA-gen_ai_cim AI Governance workshop kit `TA-gen_ai_cim_workshop/` is the
canonical reference. It lives as a **sibling app** of the target TA under
`$SPLUNK_HOME/etc/apps/` (note: it may exist under a different Splunk install
than the one you are editing — locate it with
`ls -d /opt/splunk*/etc/apps/TA-gen_ai_cim_workshop 2>/dev/null` before
assuming a path). When applying this skill, study within that kit:
- `WORKSHOP_OUTLINE.md` — six-act narrative with conditional Act 6.
- `WORKSHOP_GUIDE.md` — full click-by-click guide, including Add Data → Upload flow.
- `bin/gen_genai_events.py` — main generator.
