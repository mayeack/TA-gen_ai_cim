...
workshop on [`<APP>`](`$SPLUNK_HOME/etc/apps/<APP>`). Pair this file with
...
| **Workshop kit** | `$SPLUNK_HOME/etc/apps/<APP>_workshop/` |
...
2. App nav → **Dashboards → <Dashboard display name>** ([`<view>.xml`](`$SPLUNK_HOME/etc/apps/<APP>/default/data/ui/views/<view>.xml`)).
