...
Use `AskUserQuestion` to collect the intake below. Do NOT proceed without answers. If the user volunteers some answers in their first message, only ask the missing ones.

### Required intake questions

Ask these in ONE `AskUserQuestion` batch where possible:
...
Load the `splunk-dashboard-studio` skill (Skill tool) and build the dashboard
with it — or spawn a general-purpose subagent (Agent tool) whose prompt tells
it to read `.claude/skills/splunk-dashboard-studio/SKILL.md` first. Pass it:
...
Whoever executes it (this session with the skill loaded, or the subagent)
writes the dashboard view file and optionally deploys it.
...
If the user's answer to any Phase 1 question is ambiguous, stop and re-ask via `AskUserQuestion` — do not guess slugs, thresholds, or labels. The slug choice in particular is permanent once written to lookups, macros, and sourcetypes.
