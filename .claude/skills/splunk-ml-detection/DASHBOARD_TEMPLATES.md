...
Phase 6 renders the dashboard with the `splunk-dashboard-studio` skill. This file is the panel catalog — each panel has a title, a visualization hint, and SPL with detection tokens already substituted.

## How to build the dashboard

Load the `splunk-dashboard-studio` skill and build inline, or spawn a
general-purpose subagent (Agent tool) with a prompt that starts by telling it
to read `.claude/skills/splunk-dashboard-studio/SKILL.md` and its reference
files, followed by:
...
If the user prefers Simple XML, build a `<form version="1.1">` dashboard directly instead; the SPL and layout carry over.
...
Suggested color mapping for the dashboard build:
...
## Instructions to pass to the dashboard builder

Always include, verbatim, in the build prompt (inline or subagent):
