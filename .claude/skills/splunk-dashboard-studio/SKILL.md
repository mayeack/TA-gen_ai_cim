...
- Splunk MCP server connected for running queries and deploying dashboards
  (tools named `splunk_run_query`, `splunk_get_indexes`, etc.)
...
### Step 7: Build from a Build Spec

Once you have gathered requirements (steps 1-3) and made design decisions,
assemble the dashboard from a build spec. Either build it directly in this
session using the schema in this skill plus [viz-reference.md](viz-reference.md)
and [examples.md](examples.md), or — for large dashboards — spawn a
general-purpose subagent (Agent tool) whose prompt includes the build spec
below **and** tells it to read this skill's reference files first:
...
Whoever executes the spec (this session or a subagent) handles data discovery
via the Splunk MCP tools, JSON assembly, deployment, and validation, and
returns the complete dashboard definition plus a summary.
...
## Build Lifecycle Checklist

Whether built inline or by a spawned subagent, every build covers:
1. Data discovery via Splunk MCP (indexes, sourcetypes, fields)
2. JSON definition assembly with all sections
3. Deployment to Splunk via MCP (if available) or writing the view file
4. Structural validation (viz-to-layout, datasource refs, token consistency)

Follow the Splunk UI design standards in this skill — color palette
discipline, accessibility, and the information-hierarchy patterns under
Design Best Practices above.
