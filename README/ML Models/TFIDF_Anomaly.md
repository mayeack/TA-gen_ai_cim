...
This document provides complete SPL for training and deploying Splunk AI Toolkit TF-IDF models to detect anomalous prompts (`gen_ai.input.messages`) and anomalous responses (`gen_ai.output.messages`) in GenAI telemetry.
...
1. **Splunk AI Toolkit** (latest version)
2. **Python for Scientific Computing:** Splunk AI Toolkit ML libraries installed
...
**You CANNOT fit and apply models in the same search pipeline.** Models must be saved (via `fit ... into`) before they can be used (via `apply`).
...
Splunk AI Toolkit does not accept string values like `gamma=scale`. Either:
...
- [ ] Verify Splunk AI Toolkit is installed
