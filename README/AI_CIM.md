...
| `gen_ai.pii.ml_detected` | ML model output | Boolean: ML-based PII detection flag | `fields.conf`, `macros.conf` (genai_pii_apply_model), `savedsearches.conf` (PII ML alerts, Auto Escalate PII to Review Queue), `genai_governance_overview_studio.json` |
