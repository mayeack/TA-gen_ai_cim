#!/bin/bash
#
# Load pre-trained Prompt Injection detection model into Splunk AI Toolkit
#
...
# Check if Splunk AI Toolkit is installed
if [ ! -d "$SPLUNK_HOME/etc/apps/Splunk_ML_Toolkit" ]; then
    echo "✗ Error: Splunk AI Toolkit not found"
    echo ""
    echo "  Please install Splunk AI Toolkit from Splunkbase:"
    echo "  https://splunkbase.splunk.com/app/6842"
    echo ""
    exit 1
fi

echo "✓ Splunk AI Toolkit found"
echo ""

# Create AI Toolkit models directory if it doesn't exist
mkdir -p "$MLTK_MODELS"
...
# Copy model files from TA to AI Toolkit
echo "Copying model files to Splunk AI Toolkit..."
